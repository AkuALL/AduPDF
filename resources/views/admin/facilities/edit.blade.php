@extends('layouts.admin', ['title' => 'Edit Fasilitas — ' . $facility->name])
...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <a href="{{ route('admin.facilities.show', $facility) }}" class="text-xs font-semibold text-[#2D4C79] hover:underline flex items-center gap-1">
                ← Kembali ke Detail Fasilitas
            </a>
            <h1 class="text-xl font-bold text-slate-800 mt-2">Edit Fasilitas: {{ $facility->name }}</h1>
            <p class="text-xs text-slate-500 mt-1">
                Perbarui rincian data, tipe, kapasitas, lokasi, atau kondisi fasilitas.
            </p>
        </div>
    </div>
        ...
        <form action="{{ route('admin.facilities.update', $facility) }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                @if($facility->childTools()->exists())
                    <div class="mb-2 p-2.5 bg-amber-50 border border-amber-200 rounded text-xs text-amber-800">
                        Ruangan ini memiliki <strong>{{ $facility->childTools()->count() }}</strong> peralatan terdaftar. Tipe fasilitas harus tetap bertipe ruangan (Ruang Kelas, Aula, atau Laboratorium).
                    </div>
                @endif
                <select name="type"
                        id="type"
                    ...
                    <option value="ruang_kelas" {{ old('type', $facility->type->value) === 'ruang_kelas' ? 'selected' : '' }}>Ruang Kelas</option>
                    <option value="aula" {{ old('type', $facility->type->value) === 'aula' ? 'selected' : '' }}>Aula</option>
                    <option value="laboratorium" {{ old('type', $facility->type->value) === 'laboratorium' ? 'selected' : '' }}>Laboratorium</option>
                    <option value="alat" {{ old('type', $facility->type->value) === 'alat' ? 'selected' : '' }} {{ $facility->childTools()->exists() ? 'disabled' : '' }}>Alat (Peralatan Ruangan)</option>
                    <option value="lapangan" {{ old('type', $facility->type->value) === 'lapangan' ? 'selected' : '' }} {{ $facility->childTools()->exists() ? 'disabled' : '' }}>Lapangan</option>
                </select>
                @error('type')
                    <p class="text-xs text-rose-600 mt-1">{{ $message }}</p>
                @enderror
            ...
            <div id="parent-facility-container" class="{{ old('type', $facility->type->value) === 'alat' ? 'block' : 'hidden' }}">
                <div class="bg-amber-50 border border-amber-200 rounded-lg p-4 space-y-2">
                    ...
                    <select name="parent_facility_id"
                        ...
                        @foreach($parentRooms as $room)
                            <option value="{{ $room->id }}" {{ (int) old('parent_facility_id', $facility->parent_facility_id) === (int) $room->id ? 'selected' : '' }}>
            ...
            <div>
                <label for="condition" class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">
                    Status Kondisi <span class="text-rose-600">*</span>
                </label>
                <select name="condition"
                    ...
                    <option value="aktif" {{ old('condition', $facility->condition->value) === 'aktif' ? 'selected' : '' }}>Aktif (Siap Digunakan)</option>
                    <option value="dalam_perbaikan" {{ old('condition', $facility->condition->value) === 'dalam_perbaikan' ? 'selected' : '' }}>Dalam Perbaikan (Maintenance/Rusak)</option>
                    <option value="nonaktif" {{ old('condition', $facility->condition->value) === 'nonaktif' ? 'selected' : '' }}>Nonaktif (Tidak Dapat Direservasi)</option>
                </select>
                @error('condition')
                    <p class="text-xs text-rose-600 mt-1">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <textarea name="description"
                          id="description"
                          rows="3"
                          class="w-full text-sm rounded-md border-slate-300 shadow-sm focus:border-[#2D4C79] focus:ring-[#2D4C79] px-3 py-2 border bg-white @error('description') border-rose-500 @enderror">{{ old('description', $facility->description) }}</textarea>
                @error('description')
                    <p class="text-xs text-rose-600 mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex items-center justify-end space-x-3 border-t border-slate-200 pt-5">
                <a href="{{ route('admin.facilities.show', $facility) }}" class="text-xs font-medium text-slate-600 hover:text-slate-800 px-4 py-2 rounded-md border border-slate-300 transition">
                    Batal
                </a>
